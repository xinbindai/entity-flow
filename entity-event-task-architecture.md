...
| `taxonomy.py` | This lab's `Client`/`Patient`/`Order`/`Sample`/`Batch`/`Result` subclasses of `Entity`, plus `seed_taxonomy()` and `sync_taxonomy()` over the two CSVs. Domain configuration rather than schema, and required before any entity can be inserted; `python taxonomy.py <url>` bootstraps a dev database, `--sync` reconciles an existing one and `--dry-run` shows the diff first |
| `entity_types.csv`, `entity_statuses.csv` | The taxonomy itself — which `(category, subcategory)` pairs exist and which statuses each may hold. Two files rather than one because the tables differ in columns and cardinality: one subcategory has many statuses, and a single denormalised sheet would repeat every description and invite the copies to disagree |
...
| `entitymodel/taxonomy_sync.py` | Reads the two CSVs and reconciles `entity_type` / `entity_status` with them (§2.4): inserting what is new, updating what changed, and *reporting* rather than deleting what the files no longer mention — removal is opt-in and refused outright while any entity still uses the type or status. Idempotent, so an unchanged pair of files issues no writes |
| `entitymodel/celery_tasks.py` | The bridge from an event handler to real work (§3.6, §4.2). `submit_task()` writes the Task entity and its `TaskQueued` event in one transaction and commits *before* sending the Celery message, so a worker can never receive an id the database has not got yet; `pending_submissions()` finds the Tasks stranded when a crash lands in that gap. `entity_task()` wraps the worker body so the row follows execution — `Queued → Running → Succeeded`/`Retrying`/`Failed`, every transition through `fire_event()` |
| `entitymodel/celery_workers.py` | `WorkerFleet` — start, stop and supervise a fleet of Celery workers from a config dict of queue, concurrency and log path. The workers run as its own children rather than detached, since "SIGTERM stops the workers" only means something if a parent is alive to receive it, and liveness is read from pid files via `os.kill(pid, 0)` rather than from the broker, so a second invocation can tell that a previous run's worker is still up |
| `entitymodel/migrate.py` | Applies the packaged Alembic revisions from an installed copy: `python -m entitymodel.migrate upgrade head --db-url ...`, plus `make_config()` for driving them from a deploy script. The revisions live at `entitymodel/migrations/` rather than at the repository root because only the package reaches the wheel -- shipped separately, the models arrive without the schema they need. Alembic is an extra, since a process that only reads from the database has no use for a migration tool |
| `entitymodel/importing.py` | `import_attr()` — resolves `"package.module:attr"`, or the plain dotted form, to the object it names. The seam every configuration-driven entry point goes through: handlers listed in a settings file, the Celery app named on a command line. Raises ordinary `ValueError` / `ImportError` / `AttributeError` rather than `SystemExit`, because library code calls it as well as command lines do |
| `entitymodel/log_search.py` | `search_logs()` — filters a log file down to one handler's run, or one event's, and returns the lines the handler itself wrote along with them. Those lines go to the application's own logger and carry no handler name or event id, so what ties them to a run is position: they sit between the `entering`/`left` pair §5.3 brackets the call with. Position is only proof while one run is open at a time, so `%(process)d` in the log format is what makes it exact -- brackets are then matched per process and two interleaved workers separate cleanly. Without it a line picked up during interleaved runs is returned flagged rather than silently attributed |
...
| `test/test_cancelled.py` | `HandlerCancelled` tests (§5.3): a declined event checkpointed rather than failed, never dead-lettered, counted towards the poll's return value, its partial writes rolled back and any earlier failures cleared — plus `cancelled_reason` recorded, a cancelled event staying distinguishable from a worked one, and replay either clearing the reason or recording the new one |
| `test/test_logging.py` | Diagnostic tests (§5.3): one event followed end to end by id, each of the four reasons an empty poll can have naming itself, dead-lettering warning rather than only debugging, and the extra explanatory query not running at all when DEBUG is off |
| `test/test_trace_id.py` | Tracing tests (§3.1): `trace_id` recorded and optional, kept distinct from `actor_id` and causation, forwarded by a handler but never inherited behind the caller's back, surviving across the process boundary into the worker and onto a failing task, everything one request caused answerable in a single query, and the partial index existing and correctly scoped |
...
| `test/test_migrate.py` | Packaging tests: the revisions sitting inside the package rather than at the repository root, `env.py` and `script.py.mako` present, every revision accounted for, the chain having exactly one head and reaching base, the URL passed as an attribute so a `%` in a password survives, and `upgrade`/`downgrade` refusing to guess a target |
